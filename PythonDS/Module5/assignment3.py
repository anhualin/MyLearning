...
#
# .. your code here ..
df = pd.read_csv('C:/Users/alin/Documents/SelfStudy/PythonDS/DAT210x/Module5/Datasets/CDR.csv')
#df = df.dropna(axis = 0)
...
#
# .. your code here ..
def findOffice(df, users):
    user_work = []
    for i in range(0, len(users)):
        user1 = df[df.In == users[i]]
        user1a = user1[~user1.DOW.str.contains('Sat|Sun')]
        user1b = user1a[(user1a.CallTime < '17:00:00')] 
        user1c = user1b[['TowerLat', 'TowerLon']]
        model = doKMeans(user1c, 4)
        totalSamples =  (model.labels_==0).sum() 
        center = 0
        for j in range(1, len(model.cluster_centers_)):
            if (model.labels_==j).sum() > totalSamples:
                 center = j   
        user_work.append((users[i], model.cluster_centers_[center]))
    return user_work

user_work = findOffice(df, users)
#
...
#
# .. your code here ..
user1b.plot.scatter(x='TowerLon', y='TowerLat', c='gray', alpha=0.1, title='Weekday')
...
user1 = df[df.In == users[5]]
user1a = user1[~user1.DOW.str.contains('Sat|Sun')]
user1b = user1a[(user1a.CallTime < '17:00:00')] 
user1c = user1b[['TowerLat', 'TowerLon']]
model = doKMeans(user1c, 3)


midWayClusterIndices = clusterWithFewestSamples(model)
midWaySamples = user1b[midWayClusterIndices]
