...
rate = []
for i in range(0, 50):
    directory = 'C:/Users/alin/Documents/SelfStudy/PythonDS/DAT210x/Module5/Datasets/recordings/'
    filename = directory + '0_jackson_' + str(i) +'.wav'
    record = wavfile.read(filename)
    zero.append(record[1])
    rate.append(record[0])
# 
...
sample_rate = rate[random_idx]
wavfile.write(directory + 'Original Test Clip.wav', sample_rate, test)
...
wavfile.write(directory + 'Extrapolated Clip.wav', sample_rate, completed_clip[0])
