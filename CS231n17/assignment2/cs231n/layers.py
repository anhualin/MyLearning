...
def affine_forward(x, w, b):
    ...
    x_epd = x.reshape(x.shape[0], -1)
    out = np.dot(x_epd, w) + b
    ###########################################################################
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    s = conv_param['stride']
    pad = conv_param['pad']
    H_p = 1 + int((H + 2 * pad - HH) / s)
    W_p = 1 + int((W + 2 * pad - WW) / s)
    out = np.zeros((N, F, H_p, W_p))
    x_e = np.zeros((N, C, H + 2 * pad, W + 2 * pad))
    x_e[:,:, pad: (pad + H), pad: (pad + W)] = x
    for f in range(F):
        for n in range(N):
            for i in range(H_p):
                for j in range(W_p):
                    out[n,f,i,j] = np.sum(x_e[n,:, (i*s):(i*s+HH), (j*s):(j*s+WW)] * w[f,:,:,:]) + b[f]
    ###########################################################################
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    out1, cache =  batchnorm_forward(x2, gamma, beta, bn_param)
    out2 = (out1.T).reshape(x1.shape)
    out = np.swapaxes(out2, 0, 1)
...
def spatial_batchnorm_backward(dout, cache):
    ...
    dout1 = np.swapaxes(dout, 0, 1).reshape(C, -1).T
    dx1, dgamma, dbeta = batchnorm_backward(dout1, cache)
    dx2 = dx1.T.reshape(C, N, H, W)
    dx = np.swapaxes(dx2, 0, 1)
