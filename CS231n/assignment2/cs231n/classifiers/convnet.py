...
def mytry1(X, model, y=None, reg=0.0):
  """
  Compute the loss and gradient for 
  [conv-relu-pool] * 2 - conv-relu - affine - softmax.
  where the conv layer uses stride-1 "same"
  convolutions to preserve the input size; the pool layer uses non-overlapping
  2x2 pooling regions. We use L2 regularization on both the convolutional layer
  weights and the affine layer weights.
  ...
  - model: Dictionary mapping parameter names to parameters. 
   the model to have the following parameters:
    - W1_3, b1_3: Weights and biases for the 3 convolutional layers
...
#  W3, b3 = model['W3'], model['b3']
  ...
  conv_filter_height, conv_filter_width = W2.shape[2:]
  assert conv_filter_height == conv_filter_width, 'Conv filter must be square'
  assert conv_filter_height % 2 == 1, 'Conv filter height must be odd'
  assert conv_filter_width % 2 == 1, 'Conv filter width must be odd'
...
#
#loss0, _ = mytry1(X, model, y, reg=0)
#loss1, _ = mytry1(X, model, y, reg=1.0)
...
def init_mytry1(weight_scale=1e-3, bias_scale=0, input_shape=(3, 32, 32),
  ...
  model = {}
  model['W1'] = weight_scale * np.random.randn(num_filters[0], C, filter_sizes[0], filter_sizes[0])
